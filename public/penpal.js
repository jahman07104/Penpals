...
class Penpal {
  constructor({id, name, city, email, image_url, user}) {
    ...
    if (user) {
      // find instance of User class
      this.user = users.find(u => u.id === user.id)
    }
    penpals.push(this)
  }
  ...
  static render() {
    ...
    penpals.forEach(penpal => penpal.render())
  ...
  choose() {
    fetch(`/penpals/${this.id}/choose?user_id=${User.active.id}`,
      {method: 'POST'})
    .then(() => {
...
function renderUnchosenPenpal(penpal) {
  ...
  if (unchosenList.querySelector(`[data-id="${penpal.id}"]`)) {
    return
  }
